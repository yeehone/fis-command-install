  ...
  this.install = function(progress) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      scaffold.download(self.address + '@' + self.version, function(error, location) {
        ...
        var target = path.join(factory.settings.componentsDir, options.versionDir ? self.versions[0] : '');
        var mapping = self.config.mapping || [];

        // 给component.json 加上version 字段
        var files = fs.readdirSync(location);
        var cjsonPath = location + '/component.json';
        var cjson;
        if (files.indexOf('component.json') > -1) {
          try {
            cjson = fs.readFileSync(cjsonPath, 'utf8');
            cjson = JSON.parse(cjson);
            cjson.version = self.versions[0] || self.version;
          } catch (e) {
            console.warn('[pangu-command-install][gitlab.js][component.json add version fail]');
          }
        } else {
          cjson = {
            name: self.name || '',
        ...
        try {
          fs.writeFileSync(cjsonPath, JSON.stringify(cjson, null, '\t'));
        } catch (e) {
          console.warn('[pangu-command-install][gitlab.js][component.json write file fail]');
        }
